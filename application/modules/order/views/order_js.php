    ...
    if ($(this).prop('checked')) {
        ...
        for(i=0;i<=(row-1);i++){
            ...
            if ($('#total_belanja').val() >= 300000) {
                ...
                biaya_penanganan = total_belanja / 100;
            } else {
                ongkir = (numberOfChecked == 0 ? 0 : 5000);
                bonus = 0;
                biaya_penanganan = 0;
            }
            $('#ongkir').val(ongkir);
            ...
            $('#biaya_penanganan').val(biaya_penanganan);
            $('#label_biaya_penanganan').text('Biaya Penanganan = Rp. '+formatRupiah($('#biaya_penanganan').val()));
            
           
            $('#grand_total').val(total_belanja+ongkir-bonus+biaya_penanganan);
            $('#grand_total2').text('Rp. '+formatRupiah($('#grand_total').val()));

            // if ($('#tipe_pembayaran_select').val() == 'T') {
            //     $('#grand_total').val(total_belanja+ongkir-bonus);
            //     $('#grand_total2').text('Rp. '+formatRupiah($('#grand_total').val()));
            // }

            // if ($('#tipe_pembayaran_select').val() == 'C') {
            //     $('#grand_total').val(total_belanja+ongkir-bonus+biaya_penanganan);
            //     $('#grand_total2').text('Rp. '+formatRupiah($('#grand_total').val()));
            // }
        }
...
function getValue(radio) {
    ...
    if (vals == 'C'){
        $('#tipe_pembayaran_select').val(vals)
        $('.bukti_pembayaran').hide();
        $('.biaya_penanganan').show();
        if ($('#total_belanja').val() >= 300000) {
                biaya_penanganan = $('#total_belanja').val() / 100;
            } else {
                biaya_penanganan = 0;
            }
        $('#biaya_penanganan').val(biaya_penanganan);
        ...
        $('#grand_total').val(parseInt($('#total_belanja').val())+parseInt($('#ongkir').val())-parseInt($('#bonus').val())+parseInt($('#biaya_penanganan').val()));
        $('#grand_total2').text('Rp. '+formatRupiah($('#grand_total').val()));
    ...
    if (vals == 'T'){
        ...
        $('#grand_total').val(parseInt($('#total_belanja').val())+parseInt($('#ongkir').val())-parseInt($('#bonus').val()));
        $('#grand_total2').text('Rp. '+formatRupiah($('#grand_total').val()));
    }
}
...
function copy_norek(element) {
    ...
    toastr.success('', 'Norek telah di salin.', {timeOut: 100})
...
function cart() {
    ...
    $("#grand_total").val(0);
    $('#grand_total2').text('Rp. '+formatRupiah($('#grand_total').val()));
    if (isNaN($("#grand_total").val())){
        $("#grand_total").val(0);
        $('#grand_total2').text('Rp. '+formatRupiah($('#grand_total').val()));
    }
    if (isNaN($("#total_belanja").val())){
        $("#total_belanja").val(0);
    }
